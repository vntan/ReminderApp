...
import { deleteTag, deleteNotification, deleteSubtask, addNotification, addTag, addSubTask } from '../../../Models/tasksReducer';
import AddList from "../../AddList/AddList";

const { Option } = Select;

const EditTask = (props) => {
    const { task, showEditTask, closeEditTask, addNewTask, project, list, userID, tasks } = props
    ...
    const [taskInfo, setTaskInfo] = useState({
        ...
        idList: task.idList,
        dueDate: moment(task.dueDate, dateFormat),
        status: task.status,
    ...
    const submitNotification = () => {
        ...
        props.addNotification({ taskID: task.idTask, userID: userID, reminder: notification })
        // let listNotification = taskInfo.notification
        // let objIndex = listNotification.findIndex((item => item.reminderTime === notification))
        // if (objIndex >= 0) {
        //     return;
        // }
    ...
    const submitSubtask = () => {
        ...
        props.addSubTask({ taskID: task.idTask, nameSubTask: subTask, statusSubtask: false })
        // let listSubtask = taskInfo.subTask;
    ...
    const tickSubTask = (subtask) => {
        ...
        // listSubtask[objIndex].status = !listSubtask[objIndex].status
        // setTaskInfo({ ...taskInfo, subTask: listSubtask })
    }

    const [tags, setTags] = useState(taskInfo.tag && taskInfo.tag.map(item => {
        return item.nameTag
    }));
    ...
    const handleInputConfirm = () => {
        ...
        props.addTag({ taskID: task.idTask, nameTag: inputValue })
                ...
                >
                    <Form>
                        <Form.Item name="nameTask" rules={[{ required: true, message: 'Please input your task name!' }]}>
                            {
                                editNameTask ?
                                    <Input defaultValue={taskInfo.name} value={taskInfo.name} placeholder='Enter Name of Task' onChange={(e) => changeName(e)} onClick={() => setEditNameTask(!editNameTask)} />
                                    :
                                    <div onClick={() => setEditNameTask(!editNameTask)}>{taskInfo.name}</div>
                            }
                    ...
                    <Form layout="vertical">
                        <Form.Item label={
                            ...
                            {
                                editDescription ?
                                    <Input defaultValue={taskInfo.description} value={taskInfo.description} placeholder="Add more description" onChange={(e) => changeDescription(e)} onClick={() => setEditDescription(!editDescription)} />
                                    :
                                    <div onClick={() => setEditDescription(!editDescription)}>{taskInfo.description ? taskInfo.description : 'Add more description'}</div>
                            }
