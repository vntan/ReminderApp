...
import {
  ...
  addTag,
  addSubTask,
} from "../../../Models/tasksReducer";
import AddList from "../../AddList/AddList";

import { addTask, getTasks, updateTask } from "../../../Models/tasksReducer";
...
const EditTask = (props) => {
  ...
  const [notification, setNotification] = useState(
    !task ? [] : task.notificationList.map((item) => new moment(item.reminderTime).utc())
  );
  const [subTask, setSubTask] = useState(!task ? [] :task.subtask.map(item => ({"nameSubTask": item.name, "statusSubtask": item.status})));
  ...
  const [tags, setTags] = useState(!task ? [] : task.tag.map(item=> item.nameTag));
  ...
  const onFinish = (values) => {
    ...
    // console.log("------------------- kkkkkk", task, taskInfo);
    // console.log("Tags", tags);
    // console.log("Notificaton", notification.map(items => items.format(dateFormat)));
    // console.log("Subtasks 1", subTask);

    props.updateTask(taskInfo, tags, notification.map(items => items.format(dateFormat)), subTask, (value)=>{
        
        props.getTasks(userID, (value)=>{
            closeEditTask();
        })
      ...
      <div>
        ...
        >
          <Form
            ...
            onFinish={onFinish}
            initialValues={{
              nameTask: !task ? "" : task.name,
              project: !task || task.idProject === null ? -1 : task.idProject,
              list: !task || task.idList === null ? -1 : task.idList,
              deadline: !task || task.dueDate === null ? -1 : new moment(task.dueDate),
              description: !task || task.description === null ? "" : task.description,
            }}
          >
            <Form.Item
              name="nameTask"
            ...
            <Row style={{ marginBottom: "-20px" }}>
              <Col span={12}>
                <Form.Item
                  ...
                  initialValue={-1}
                >
                  <Select listHeight={130} onChange={changeProject} disabled>
            ...
            >
              <Select
                style={{
                  ...statusToColor(form.getFieldValue("progress")),
                  width: "205px",
                }}
                showArrow={false}
              >
